...
namespace Measurer
{
    public class ExcelHelper
    {
        ...
        static MemoryStream ExportDT(DataTable dtSource, string strHeaderText)
        {
            ...
            foreach (DataRow row in dtSource.Rows)
            {
                ...
                if (rowIndex == 65535 || rowIndex == 0)
                {
                    ...
                    {
                        HSSFRow headerRow = sheet.CreateRow(1 + offset) as HSSFRow;
        ...
        /// <returns></returns>
        public static bool MergedCell(string outputFile, int col)
        {
            FileStream readfile = new FileStream(outputFile, FileMode.Open, FileAccess.Read);

            HSSFWorkbook hssfworkbook = new HSSFWorkbook(readfile);
            ISheet sheet1 = hssfworkbook.GetSheetAt(0);    // GetSheet(sheetname);

            try
            {
                string firstVal = string.Empty, lastVal = string.Empty;
                int firstIdx = 0, lastIdx = 0;

                IEnumerator rows = sheet1.GetRowEnumerator();
                while (rows.MoveNext())
                {
                    IRow iRow = (HSSFRow)rows.Current;
                    ICell iCel = iRow.Cells[col];

                    lastIdx = iCel.RowIndex;
                    lastVal = iCel.ToString();
                    if (lastVal == firstVal)
                    {
                        sheet1.AddMergedRegion(new CellRangeAddress(firstIdx, lastIdx, col, col));
                    }
                    else
                    {
                        firstIdx = iCel.RowIndex;
                        firstVal = iCel.ToString();
                    }
                }
            }
            catch
